...
module.exports.beta = async(client, message, args) => {
    ...
    message.channel.send(new MessageEmbed().setColor('BLUE').setAuthor(message.author.username, message.author.avatarURL({ dynamic: true})).setDescription(`${uye} Adlı kişi ${message.author} tarafından <@&${config.Register.e1}> Rolü verilerek kayıt edildi. \n  Kişinin yeni ismi: \`${İsim}\``))
    client.channels.cache.get(config.Log.Sohbet).send(`${uye}, Aramıza Katıldı.`)
    client.channels.cache.get(config.Log.RegisterLog).send(new MessageEmbed().setAuthor(message.member.nickname, message.author.avatarURL()).setColor('BLUE').setTitle("KAYIT [ERKEK]").setDescription(`• Yetkili: ${message.author} (\`${message.author.id}\`) \n • Kullanıcı: ${uye}(\`${uye.id}\`) \n • Verilen Roller: <@&${config.Register.e1}>,<@&${config.Register.e2}>,<@&${config.Register.e3}> \n • Tarih: \`${kayıttarihi}\``))
